...
function produce() {
	...
	for (let part in inventory.parts) {
		...
		let amount = Math.min(
			factories_can_produce,
			u(`#produce-${part}-range`).first().value
		);
	...
	u('.production-total').text(
		inventory.buildings.factories - factories_can_produce
	);
	...
	for (let building in inventory.buildings) {
		...
		if (tick_num % 40 == 0) {
			// only do craft every 40 ticks, but still validate ranges every tick
			if (amount != 0) craft(building, amount);
			u('#next-craft').html(
				'<p>Time until next craft:</p> ' + getProgressbar(10 * 1000)
			); // show progress bar for 10s
		}
	}
	u('.crafting-total').text(
		inventory.buildings.crafters - crafters_can_craft
	);
...
function display() {
	...
	for (let part in inventory.parts) {
		...
		html +=
			...
			used +
			' space ' +
			getStillProgressbar((used / max) * 100) +
			'<br>';
	}
	html +=
		'Remaining: ' +
		remaining +
		' space ' +
		getStillProgressbar((remaining / max) * 100) +
		'<br>';
...
function setup() {
	...
	html += '<p>Craft:</p>';
	html +=
		'<select id="craft-amount-select"><option value="1">1</option><option value="10">10</option><option value="100">100</option></select>';
	for (let building in inventory.buildings) {
		html += `<button onclick="craft('${building}', parseInt(u('#craft-amount-select').first().value) )">Craft ${building} ${getSprite(
			building,
			'md'
		)}</button>`;
	}
	...
	for (let building in inventory.buildings) {
		html += `<span id="craft-${building}-value"></span> <input id="craft-${building}-range" type="range" value="0" step="1" min="0" max="1"> ${getSprite(
			building,
			'md'
		)} ${capitalize(building)}<br>`;
	}
	...
	html =
		getSprite('warehouses', 'lg') +
		'<h3>Storage</h3><p><span class="warehouses"></span> warehouses</p>';
	...
	html =
		getSprite('converters', 'lg') +
		'<h3>Conversion</h3><p><span class="converters"></span> converters</p>';
	html += '<p>Converters convert 1 part into any other 1 part per tick</p>';
	html += '<p>Convert:</p>';
	html +=
		'<span id="convert-value"></span> <input id="convert-range" type="range" value="0" step="1" min="0" max="1"><br>';
	...
	for (let recipe in recipes) {
		...
		for (let part in recipes[recipe]) {
			...
			html += `${recipes[recipe][part]} ${
				part != 'time' ? part : 'ticks'
			} ${getSprite(part, 'sm')}, `;
		}
	...
	html = `<div class="grid grid-cols-1 md:grid-cols-2"><div class="m-2 p-2 border-2 border-gray-300 bg-white bg-opacity-25">
		<h3 class="my-4 font-bold">${getIcon('parts')} Parts</h3>`;
	for (let part in inventory.parts) {
		html += `<span>${getSprite(
			part,
			'md'
		)} <span class="${part}"></span> ${capitalize(part)}<span>, `;
	}
	html = html.slice(0, -2); // remove trailing ", "
	html += `</div><div class="m-2 p-2 border-2 border-gray-300 bg-white bg-opacity-25"><h3 class="my-4 font-bold">${getIcon(
		'buildings'
	)} Buildings</h3>`;
	for (let building in inventory.buildings) {
		html += `<span>${getSprite(
			building,
			'md'
		)} <span class="${building}"></span> ${capitalize(building)}<span>, `;
	}
	html = html.slice(0, -2); // remove trailing ", "
	html += '</div></div>';
	...
	html =
		getSprite('factories', 'lg') +
		' <h3>Production</h3><p><span class="factories"></span> factories</p>';
	html += '<p>Factories produce 1 part/tick or 2 gears/tick</p>';
	for (let part in inventory.parts) {
		html += `<span id="produce-${part}-value"></span> <input id="produce-${part}-range" type="range" value="0" step="1" min="0" max="1"> ${getSprite(
			part,
			'md'
		)} ${capitalize(part)}<br>`;
	}
	...
	for (let category in inv) {
		for (let key in inv[category]) {
			if (
				...
				inventory[category][key] != undefined
			) {
